...
//
//	// 当容器存对象比较大，或者这个对象要做深拷贝，如string最好给&和const，可以减少拷贝提高效率
...
// final与override 
// 这两个关键字分别的作用是什么
// final修饰类，类就变成了最终类，不能被继承  final还可以修饰虚函数，这个虚函数不能被重写
...
// 新容器
// C++98容器
// string/vector/list/deque/map/set/bitset + stack/queue/priority_queue 旧容器
// C++11新容器
...
public:
	...
	// s2(s1)
	String(const String& s)//空间越大 值越多 效率越低	
	{
...
//{
//	String tmp(str);
//	return tmp; // 这里返回实际是拷贝tmp的临时对象 tmp的生命周期到了 无法返回
//}
...
//template <class... Args>
//void emplace_back(Args&&... args);   // 模板的可变参数 
...
// 实现原理：封装库时使用了条件编译，底层分别调用了不用平台的线程API
